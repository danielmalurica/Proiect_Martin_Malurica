...
export default class EditCalatorie extends Component {
  ...
  constructor(props) {
   ...
   componentDidMount() {
    axios.get('http://localhost:5000/calatorie/'+this.props.match.params.id)
      .then(response => {
        this.setState({
      punctPlecare: response.data.punctPlecare,
      punctSosire: response.data.punctSosire,
      mijlocTransport: response.data.mijlocTransport,
      durataCalatorie: response.data.durataCalatorie,
      gradAglomerare: response.data.gradAglomerare,
      oraDataPlecare: new Date(response.data.oraDataPlecare),
      observatii: response.data.observatii,
      nivelSatisfactie: response.data.nivelSatisfactie,
        })   
      })
  ...
  onSubmit(e) {
    ...
    axios.post('http://localhost:5000/calatorie/update/'+this.props.match.params.id, calatorie)
  ...
  render() {
    ...
    return (
      <div className="addCalatorieForm">
        <h3>Introduceți datele de mai jos pentru a edita:</h3>
        <form onSubmit={this.onSubmit}>
          ...
          <div>
            <input type="submit" value="Editeaza călătorie"  />
          </div>
        </form>
      </div>
    )
